...
CHSV vleds[nSegs];
const int nLimits = 5; 
int limits  [nLimits] = {1,2,4,5};
int limit2seg [nLimits+1] = {0,1,2,3,4};
...
void setup() {
  ...
  //initialize with color objects
  for (int i=0;i<nSegs;i++){
    vleds[i] = CHSV(0,255,0);
  }
...
SimplePatternList gPatterns = {beamClass};
...
void beamClass(){
  Beam b1 = Beam(vleds,nSegs,0,1,2,10);
  int tau=1;
  //b1.fire();
  //fire();
  b1.off();
  while (tau<1000){//change here to make it shorter
    //b1.move(tau);
    //b1.off();
    b1.cross(tau);
    fire(200);
...
int getDelay(int tau){
  int tempi=10;
  return 50 + (int(floor(tau/50))%tempi)*50;
}

void bouncingBeam2(int start)
{
...
/*const int nSegs = 5;
CRGB vleds[nSegs];
const int nLimits = nSegs-1; 
int limits  [nLimits] = {1,2,3,4};
int limit2seg [nLimits+1] = {0,1,2,3,4};
*/
{
  for (int l=0;l<=nLimits;l++)
  {
    if (rled < limits[l]) //limits = {1,2,3,4};
    {
      return limit2seg[l]; //limit2seg= {0,1,2,3,4};
    }
  } 
}

//void fadingBeam(int breite=2, int speeed=10){
void fadingBeam(){
  int breite=2;
  int speeed=20;
  //int d = 1; // direction: either -1 or +1
  ...
  int offLED;
  int prevSeg=0;
  CHSV front = CHSV(gHue,255,5);
  CHSV back = CHSV(gHue,255,5);
  while (1 or (tau<1000)){
    prevSeg=onLED;
    ...
    if (onLED<nSegs){ 
      ...
      //front.val = min(255,front.val+inc);
      vleds[onLED] = front;
    }
    if ((onLED-breite)>=0){
...
void bouncingBeam(int start)
{
     ...
     vleds[offIndex].val=0;//CRGB::Black;
     fire(500);
     //cout << onIndex<< "  "<<offIndex << endl;
     }
}
...
void beam(){
  for (int i=0; i<nSegs+1; i++)
  {
    if (i==0)
    {
      ...
      vleds[nSegs].val=0;//=CRGB::Black;
      fire();
    }
    else
    {
      vleds[i]= CHSV(gHue, 255, 255);
      vleds[i-1].val=0;//::Black;
      fire();
    }
  }
}
