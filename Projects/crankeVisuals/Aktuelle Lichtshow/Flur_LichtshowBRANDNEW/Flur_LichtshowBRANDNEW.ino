...
SimplePatternList gPatterns = {smashingBeams, bouncingBeam, confetti,fadingBeam, sinelon, juggle, beam, theaterChase, theaterChaseRainbow};
...
int getDelay(int tau){
  int tempi=10;
  return 5 + (int(floor(tau/50))%tempi)*50;
}
void bouncingBeam()
{
     ...
     while (tau<3000){ //use to make it shorter or longer
...
void smashingBeams(){
  Beam b1 = Beam(vleds,nSegs,0,1,5,180);
  int tau=1;
  b1.off();
